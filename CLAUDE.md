...
`picocalc.sys.applyUpdate(path)` is registered only when the app declares `"system-update"` AND is an OS app: its id is `com.picos.updater` or `com.picos.store`, or its directory is under `/system/`. (Requirements are consent, not a boundary — an app writes its own `app.json` — so the id/location check narrows it.) It checks the path against the sandbox, validates the image and requires `/system/update.sha256`, then shows a `ui_confirm` naming the file before rebooting into the updater; declining returns `false, "cancelled"`. The boot-time updater refuses to flash when the `.sha256` file is missing or does not match. **The boot-time gate is a one-shot OTA token** (`OTA_MAGIC` in watchdog scratch, set only by C in `ota_trigger_update` — after `applyUpdate`'s confirm, or by the `reboot-ota` dev command that `make flash-ota` sends): a `/system/update.bin` found at boot without the token is never flashed; it is renamed to `update.bin.stale` (with its `.sha256`) and logged to `/system/error.log`. `ui_confirm` discards input queued before it appears (STM32 FIFO, injected keys), ignores input for 400 ms, and needs a fresh Enter press edge (or `y`), so an app cannot pre-load a "yes".
