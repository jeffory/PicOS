...
To stage a multi-file app on hardware or the simulator, prefer the `push_app` MCP tool (`tools/picos_mcp.py`): it ships the whole directory as one ZIP and extracts it on-device via the `unzip <zip> <dest>` dev command (`rm <path>` cleans up), far faster than per-file transfers for asset-heavy apps. Dev commands run on an app stack (a 32 KB PSRAM stack at the launcher, inline on the app's stack inside an app), so `unzip` is safe at the launcher; `stack` reports the last launcher command's peak (`os_cmd_peak`, ~5.3 KB for a 66-file unzip). `exit` with no app running replies `Error: exit: no app running`; it still raises the exit flag, so a modal open at the launcher (system menu, text input) closes, and the launcher then drops it (it used to return from `launcher_run` and HardFault). While an app runs (`src/dev_commands.c`, `lua_bridge.c`, native `sys_poll`): plain `reboot` and `reboot-flash` are honoured at once and kill the app without teardown (a native app that never calls `sys->poll()` latches them until the launcher); `reboot-ota` is dropped (`reboot-ota ignored`), not deferred, so `make flash-ota` (`tools/ota_flash.py`) exits the app first, and a staged image rebooted into without the OTA token is renamed `update.bin.stale`; `usb` waits for the launcher. In the simulator the `dev_command` RPC (`{"cmd": "unzip ..."}`) runs `ping`/`exit`/`unzip`/`rm` through the same handlers (`src/dev_ops.c`).

Tests: the simulator E2E suite (`SDL_VIDEODRIVER=dummy pytest tests/e2e -n auto`; see `tests/e2e/README.md`, config and markers in the repo-level `pytest.ini`). It runs against `build_sim/picos_simulator` by default; `PICOS_SIM_BINARY` (or `--simulator-path`) selects another build: `make simulator-asan` (`build_sim_asan/`, ASan+UBSan; enables the `asan_only` tests), `make simulator-tsan`, and `make simulator-net` / `-net-asan` / `-net-tsan` (`build_sim_net/`…, the firmware network stack; enables the `firmware_net` tests). `--target hw:/dev/serial/by-id/<PicOS device>` runs the `hardware`/`both` tests on a real PicoCalc through `tests/e2e/hw_target.py` (they skip, allow-listed, on the simulator); `virtual_time=True` per test (or `PICOS_SIM_VIRTUAL_TIME=1`) runs a test-mode sim on a virtual clock, and `--real-umm` (sim flag) swaps in the firmware allocator — both under Simulator Notes. Also `make test-unit` (host C unit tests: `tests/unit/CMakeLists.txt` → `build_unit/`, ctest, ASan+UBSan; covers the pure modules `elf_plan`, `fs_path`, `app_manifest`, `config`/`appconfig` over an in-memory SD fake, `wav`, `audio_ring`, `zip_name`, `text_wrap`, `lua_numfmt` vs glibc, fonts), `python3 -m pytest tests/unit` and `make test-lua`. `make fuzz` runs the libFuzzer targets in `tests/fuzz/` (clang; `FUZZ_TARGETS=`, `FUZZ_SECONDS=`); CI runs them nightly (`.github/workflows/fuzz.yml`) and the unit job on every push (`unit.yml`). There is no linter. Lua fixture apps report through the test kit `tests/e2e/lib/picotest.lua` (staged as `/system/lib/picotest.lua`).
...
- `picocalc.tcp` / `g_api.tcp` — raw TCP/TLS sockets (native: connect/connectEx/write/read/close/available/getError/…; Lua: `picocalc.tcp.new(host, port, tls)` objects with `connect`/`write`/`read`/`close`/`available`/`error`/`getEvents`/callbacks)
...
5. Dispatches via `AppRunner` vtable (`src/os/app_runner.h`):
   ...
   - Exit teardown. Launcher, around every app (both kinds): full-screen clip rect, scroll offset 0, font 0 and `font_registry_unload_all()`, `system_menu_clear_items()`, `zip_archive_close_all()`. Lua runner: `lua_close()` (every userdata `__gc`), then leftover files (`app_files_close_all`), tone/stream stop, `fileplayer_reset`, `sound_init`, `mp3_player_reset`, and Core 1 resumed if the app left it paused (`sys.pauseBackground`, resumed before `lua_close`); REPL scrollback freed; keyboard state cleared at launch (`kbd_clear_state`, as the native loader does). Native runner: every object the app got through the API is tracked against it — files in `app_files_*`, images/samples/sample players/file players/MP3 players/video players/MOD players/terminals/AES and ECDH contexts/`qmiAlloc` blocks in `app_res_*` (`app_identity.h`, kinds in `native_loader.h`) — and after the app returns the loader frees whatever is left (`native_res_release_all`), then does the same audio resets, `http_close_all`/`tcp_close_all` (the pools, while Core 1 runs), clears the audio callback and drops a latched `reboot-ota`. Firmware does the tracking through a per-launch PSRAM copy of `g_api` whose create/free entries adopt/drop the handle (same layout; every other entry is `g_api`'s); the simulator's Unicorn trampolines call the same `native_res_adopt`/`native_res_drop`/`native_file_*`. A free of a pointer that is not tracked (a double free, a stray pointer) is ignored — but only while that address is not tracked again: once the allocator has reused it for a newer object of the same kind, a stale free releases that newer object
...
- `LUAI_MAXSTACK` counts stack *slots*, not frames: a typical frame costs 10–13 slots, so recursion tops out around 80–100 levels. 500 (the old nominal value) overflowed the minesweeper flood fill on ~3% of first clicks; 1000 is the smallest value that ran every app in the tree when it was set (minesweeper was then untracked and checked in the simulator only).
...
A count hook (`lua_sethook` with `LUA_MASKCOUNT`) runs `lua_bridge_service_poll()`. Its count adapts between 128 and 4096 instructions so it fires about once per ms of wall time (large in compute-bound code, small in apps that spend their time in C calls). Every call feeds the watchdog and checks the exit request and the Sym (Menu) key; the full service pass (HTTP/TCP/sound callbacks, the serial dev-command poll, reboot flags, screenshots, low-memory GC) runs only when `g_lua_service_pending` is set (Core 1's sound trampolines, the simulator's control socket; `lua_bridge_request_service()`) or 5 ms have passed since the last one. `input.update()` and `display.flush`/`flushRows`/`flushRegion` run the same gated pass (so a draw loop is served once a frame whatever the count); `sys.sleep` (every 10 ms, after a `kbd_poll_background()` that queues keys without consuming the app's edges) and the terminal widget's blocking waits run the full pass (`lua_bridge_service()`), so all of them honour the menu, callbacks, dev commands and the exit request. The count is per Lua thread (read back with `lua_gethookcount`).
...
- Colors are RGB565 host-byte-order; the framebuffer stores them **byte-swapped** (panel order: the 8-bit PIO SPI DMA sends bytes as stored and the ST7365P wants big-endian), so every primitive swaps on write and every reader (screenshot, effects, `display_darken`, the MCP screenshot decoder) swaps back. Native apps writing via `getBackBuffer()` must write swapped pixels too (the documented contract). Moving to a native-order framebuffer with `channel_config_set_bswap` on the LCD DMA was scoped and **not** done: it breaks that native ABI (TIC-80, C64, DOS86 in-tree; DOOM, C-Dogs, Rally out of tree; GBC is not affected — it draws through `drawImageNN` + `flushRows`), JPEGDEC's `RGB565_BIG_ENDIAN` video path, `display_blit_be`, terminal_render and the screenshot tools — see the Task 18 report
- **Clip contract**: every primitive clips its destination to the clip rect (`setClipRect`, default the screen) **once**, in int64, before any pixel loop — `drawLine(0,0,1e9,1e9)`, a radius-1e9 circle or a huge triangle cost O(visible pixels), and a primitive's own coordinates/sizes may be any int. Not covered by the int64 clip (keep these in range; Lua already rejects |v| > 2^24): `drawScaledNN`'s `img->w * scale` product, the tgx rotate path's float→int conversion, and the textured column's `y1 - y0 + 1`. The rasterisers live in `src/drivers/display_clip.h`, shared by display.c (`swap = true`) and the simulator's driver_stubs.c (`swap = false`) and host-tested (`tests/unit/test_display_clip.c`) against the old per-pixel loops. Image blits run one of four specialised loops (opaque/keyed × flip/no-flip); a colour key of 0 means opaque. Native `drawImageNN` clips once too and draws the visible part of a scaled block that straddles the clip edge (it used to write row −1 at `y=-3, scale=2`). `drawScaledNN` samples column/row `d * src / dst` exactly (remainder DDA); the old 16.16 ratio sampled the wrong source column for every scale that is not a power of two (visible at multiples of 3 for scale 3); firmware now matches the simulator. Exceptions: `display_clear`, the effects and flushes (whole buffer by design); the tgx rotate path clips through a sub-view
...
- `sound.c` — sound sample loading and playback. All WAV loaders share one RIFF chunk walker (`src/drivers/wav.c`, host-tested + fuzzed): 8/16-bit PCM, 1-2 channels, EXTENSIBLE resolved; float, 24/32-bit, ADPCM and >2 channels are refused. Samples keep the first 64 KB of data; the fileplayer streams **16-bit only** (an 8-bit WAV is refused there)
...
- Layout (`pio_psram.h`): MP3 PCM ring `0x0000` (32KB), a video region `0x8000` (256KB) that is reserved for the OS but unused (the video frame pool lives in QMI PSRAM, see Video Player), apps from `PIO_PSRAM_APP_BASE` = `0x48000` (288KB)
...
- **QMI PSRAM (8MB)**: Lua heap via `umm_malloc` at 0x11200000 (cached alias), 6 MB minus the 128 KB Core 1 pool. ELF app data/BSS. Never mix `umm_malloc`/`umm_free` with standard `malloc`/`free`.
...
Native apps receive `PicoCalcAPI *api` as their entry point argument. Use `api->sys->poll()` each frame and check `api->sys->shouldExit()` to handle the system menu exit. Anything the app does not free (files, images, players, terminals, crypto contexts, `qmiAlloc` blocks, HTTP/TCP connections, loaded fonts, menu items, zip handles) is freed when `picos_main` returns. Freeing a handle twice is ignored only until its address is reused by a newer handle of the same kind (then the second free releases the newer one), so do not rely on it.
...
- HardFault record (`hardfault_c` in `src/main.c`; layout comment above `CRASH_TAG`): the SDK's `watchdog_reboot(0,0,0)` zeroes scratch[4], so what must survive is in scratch[0-3] — [0] `CRASH_TAG` | flags (booting + that boot's attempt number, PSP, PSP = Lua VM / OS command, HFSR forced/vecttbl) | SFSR, [1] stacked PC (outside a record: the one-shot OTA token, zeroed on the crash path), [2] stacked LR, [3] CFSR — plus best-effort [5] pre-fault SP, [6] fault address or a diagnostic pack, [7] app uptime (s). After a stacking-time STKOF the stacked PC/LR can be garbage. The fault screen is held ~3 s, then the device reboots.
- Boot-loop protection: while booting scratch[0] holds `BOOT_MAGIC | attempt`; a boot that faults or hangs until the watchdog counts as a failed attempt, and reaching the launcher resets it. After `BOOT_MAX_RETRIES` (3) failures the boot watchdog stays off, and a fault at the limit shows "Boot failed N times - halted" and halts instead of rebooting; power-cycling (batteries out — USB replug keeps the scratch registers) clears it.
...
- `picocalc.sys.getMemInfo()` returns `psram_largest_block` and `psram_fragmentation` alongside the totals, plus the Lua small-object pool figures (`small_pool_*`).
...
conn:setInsecure(bool)                -- before get/post: skip TLS verification + clock gate (dev servers only)
conn:get(path, [headers])             -- false while a request is in progress
...
There is no `conn:query` (older docs called it an alias for `post`; it was never registered). Other names that differ from older docs: the TCP socket's error getter is `tcp:error()` (not `getError`; the native `g_api.tcp->getError` keeps its name), and `picocalc.video` players have no `destroy()` (the collector frees them). There is no `graphics.cache` table; sprite `setImageDrawMode`, `setIgnoresDrawOffset`, stencils, sprite clip rects, `addDirtyRect` and `markDirty` are accepted but not applied.
...
- `--test-mode` determinism: `math.random` is seeded with a constant at every app start (`lua_bridge.c`, `#ifdef PICOS_SIMULATOR`), Lua's string hash seed is constant (so `pairs()` order over string keys repeats; `simulator/sim_lua_seed.h`), and the clock is pinned to 2026-01-01T00:00:00Z at boot (`getClock` reports `synced=true`; it then runs with the sim clock). `get_running_app` always returns `{running, name|null}`.
- Virtual time (`--virtual-time`, needs `--test-mode`; `PicosSimulator(virtual_time=True)`; `PICOS_SIM_VIRTUAL_TIME=1` turns it on for every test-mode sim): the clock the OS and apps read (`getTimeMs`, `to_ms_since_boot`, the input hold, animators) is virtual. Only Core 0 (the main thread) moves it: `sleep_ms(n)` advances it by `n` and waits `n / multiplier` real ms (default 50x, so `sys.sleep(5000)` costs ~100 ms and reads back exactly 5000); a stretch of more than 50 ms without a sleep advances it at `min(multiplier, 1)` x real time; `set_time_multiplier 0` pauses it and `step_time {ms}` (returns `{now_ms}`) moves it, a paused `sys.sleep` returning only as it is stepped. Core 1 and the socket thread sleep in real time, and audio (SDL, wall-clock ring drain) and sockets stay real-time, so network timeouts measured in virtual time would fire early: it is opt-in per test and the audio/network suites stay on the wall clock. Without it the multiplier only scales `hal_sleep_ms`.
...
- `umm_*` is a counting allocator over host malloc: `umm_free_heap_size()` / `get_heap_info` report 8 MB minus the live umm/Lua bytes, but the largest free block is approximated by the free total and fragmentation is always 0 (use `--real-umm` for those). ASan builds poison free small-pool slots and the slack past each request, so overflows inside a slab are still caught.
- `--real-umm` runs `umm_*` on the firmware's own `umm_malloc` (device heap size, 200-byte blocks) instead, so block waste, fragmentation and the largest free block behave as on hardware (ASan no longer sees inside the heap). On the 64-bit host Lua objects are about twice their device size, so the small-object pool classes double too (up to 256 B).
