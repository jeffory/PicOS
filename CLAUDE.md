...
- Timeouts (`http_check_timeouts` / `tcp_check_timeouts`) run on every Core 1 tick, link up or not; the link going down (`MG_TCPIP_STATE_DOWN`) fails every connection in flight with `"network down"`. `wifi_hw_disconnected()` turns true on that link-down event after a requested disconnect (or 1.5 s without one), not when `mg_wifi_disconnect()` returns
