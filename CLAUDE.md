...
- Bridge quantity arguments (coordinates, sizes, durations, volumes, rates, frame indices; `lb_checkint`/`lb_optint` in `lua_bridge.c`) round floats to nearest (ties toward +inf), so `199.99998` draws at 200; NaN/inf and floats beyond ±2^24 are argument errors. Identifiers (handles, enums, colours, masks, byte counts, ports) still require exact integers; the `display` primitives that always read floats (`fillRect`, `drawLine`, `drawText`, …) still truncate toward zero.
