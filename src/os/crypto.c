...
void crypto_hmac_sha256(const uint8_t *key, uint32_t klen,
    ...
    if (!md_info) return;
    mbedtls_md_hmac(md_info, key, klen, data, dlen, out);
}
...
void crypto_ecdh_get_public_key(crypto_ecdh_t *ctx,
                                uint8_t *out, uint32_t *out_len) {
    if (!ctx || !ctx->valid || !out || !out_len) return;

    if (ensure_rng() != 0) { *out_len = 0; return; }

    // make_public is idempotent once keypair exists; call it unconditionally
    uint8_t tls_buf[128];
    size_t olen = 0;
    int ret = mbedtls_ecdh_make_public(&ctx->ctx, &olen,
                                        tls_buf, sizeof(tls_buf),
                                        mbedtls_ctr_drbg_random, &s_ctr_drbg);
    if (ret != 0) { *out_len = 0; return; }

    ctx->has_keypair = true;

    // Strip the TLS length byte: tls_buf[0] = length, tls_buf[1..olen-1] = point data
    uint32_t data_len = (uint32_t)(olen - 1);
    if (data_len > *out_len) data_len = *out_len;
    memcpy(out, tls_buf + 1, data_len);
    *out_len = data_len;
}
