...
static void collect_cb(const sdcard_entry_t *e, void *user) {
    ...
    if (e->name[0] == '.') return;   // skip hidden / system entries
...
static void draw_browser(const char *path, int sel, int scroll) {
    ...
    // Divider below title
    display_fill_rect(px + 1, py + 1 + FB_TITLE_H, FB_PANEL_W - 2, 1, FB_C_BORDER);
    ...
    for (int i = 0; i < visible; i++) {
        ...
        if (max_name > (int)(sizeof(label) - 2)) max_name = (int)sizeof(label) - 2;
    ...
    display_fill_rect(px + 1, footer_y, FB_PANEL_W - 2, FB_FOOTER_H, FB_C_TITLE_BG);
    display_draw_text(px + 4, footer_y + 2,
                      "Enter:open  Esc:up/cancel", COLOR_GRAY, FB_C_TITLE_BG);
}
